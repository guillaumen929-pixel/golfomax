...
function VideoHero() {
  ...
  useEffect(() => {
    ...
    // Seek once on mount in case page loaded mid-scroll
    seek()
  ...
  // Reduced-motion: single viewport section, no scroll scrub
  if (reducedMotion) {
    return (
  ...
  // Scroll-scrubbed version: outer section is 300vh tall, inner sticks at top
  return (
    ...
    <section style={{ background: '#1C1C1E', padding: '6rem 1.5rem', position: 'relative', zIndex: 1 }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <motion.div
          ...
          style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1px', background: '#2E2E2E' }}
        >
          {t.features.items.map((item, i) => (
            <motion.div
              key={i}
              variants={fadeUp}
              style={{ background: '#1C1C1E', padding: '2.5rem 2rem', position: 'relative', overflow: 'hidden' }}
            >
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: '#C1272D', opacity: 0.4 }} />
              <span style={{ display: 'block', fontFamily: 'var(--font-display)', fontSize: '2.5rem', color: '#2E2E2E', lineHeight: 1, marginBottom: '0.75rem' }}>
                {String(i + 1).padStart(2, '0')}
              </span>
              <h3 style={{ color: '#fff', fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '1rem', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '0.6rem' }}>
                {item.title}
              </h3>
              <p style={{ color: '#999', fontFamily: 'var(--font-body)', fontSize: '0.875rem', lineHeight: 1.65 }}>
                {item.desc}
              </p>
            </motion.div>
          ))}
