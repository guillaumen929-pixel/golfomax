...
export default function BookingForm() {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', date: '', time: '', players: '', package: '', message: '' })
  const [status, setStatus] = useState('idle')
  const [errors, setErrors] = useState({})

  const required = ['name', 'email', 'date', 'time', 'players']
  ...
  return (
    <section id="booking-form" className="section-padding bg-bg-surface">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="text-center mb-10">
            <SectionLabel>{t.booking.cta}</SectionLabel>
            <h2 className="font-display text-display-md text-white">{t.booking.formTitle}</h2>
          </div>
              ...
              <motion.form key="form" onSubmit={submit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="date" className={`${inputClass} ${fieldErr('date')}`} value={form.date} onChange={set('date')} />
                  </div>
                  <div>
                    <label className={labelClass}>{f.time}</label>
                    <input type="time" className={`${inputClass} ${fieldErr('time')}`} value={form.time} onChange={set('time')} />
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className={labelClass}>{f.players}</label>
                    <select className={`${inputClass} ${fieldErr('players')}`} value={form.players} onChange={set('players')}>
                      <option value="">—</option>
                      {[1,2,3,4,5,6].map(n => <option key={n} value={n}>{n}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className={labelClass}>{f.package}</label>
                    <select className={inputClass} value={form.package} onChange={set('package')}>
                      <option value="">—</option>
                      {t.packages.items.map(p => <option key={p.name} value={p.name}>{p.name}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea className={`${inputClass} resize-none`} rows={3} value={form.message} onChange={set('message')} placeholder={f.message} />
                </div>
